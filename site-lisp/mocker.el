...
(cl-defmethod make-instance ((mock (subclass mocker-mock)) new-name &rest args)
...
(cl-defmethod mocker-add-record ((mock mocker-mock) &rest args)
  (object-add-to-list mock :records
                      (let ((cls mocker-mock-default-record-cls)
                        ...
                        (when tmp
                          (setq cls tmp)
                          (setq args (mocker-read-record cls (mocker--plist-remove args :record-cls))))
                        (apply 'make-instance cls :-mock mock :-sym (make-symbol "unique") args))
                      t))
...
(defclass mocker-record-base ()
          ((min-occur :initarg :min-occur :initform 1 :type number)
           ...
           (-occurences :initarg :-occurences :initform 0 :type number :protection :protected)
           (-mock :initarg :-mock)
           (-active :initarg :-active :initform t :protection :protected)
           (-sym :initarg :-sym)))

(cl-defmethod make-instance ((rec (subclass mocker-record-base)) new-name &rest args)
  (let* ((obj (cl-call-next-method))
...
(cl-defmethod mocker-use-record ((rec mocker-record-base))
  (let ((max (oref rec :max-occur))
    ...
    (if (and max (> n max))
        (signal 'mocker-record-error
                (list (format "Unexpected call to mock `%s'" (oref mock :function))))
...
(cl-defmethod get-record-expectations ((rec mocker-record-base)))

(cl-defmethod mocker-failed-record ((rec mocker-record-base) args)
  (signal 'mocker-record-error
...
(cl-defmethod make-instance ((rec (subclass mocker-input-record)) new-name &rest args)
  (let* ((obj (cl-call-next-method)))
...
(cl-defmethod mocker-run-record ((rec mocker-record) args)
  (let ((generator (oref rec :output-generator))
    ...
    (cond (generator (apply generator args))
          (t output))))
...
(cl-defmethod make-instace ((rec (subclass mocker-stub-record)) new-name &rest args)
  (let* ((obj (cl-call-next-method)))
