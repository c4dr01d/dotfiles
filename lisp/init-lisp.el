...
(use-package elisp-mode
  ...
  :hook ((emacs-lisp-mode . (lambda ()
			      (setq-local lisp-indent-function #'local/lisp-indent-function)))
	 (help-mode . cursor-sensor-mode))
  :config
  (defun local/lisp-indent-function (indent-point state)
	...
	(insert-button
	 ...
	 'action (lambda (_)
		   (when (yes-or-no-p msg)
		     (message "%s from fucntion `%s'" msg function)
...
(use-package helpful
  ...
  :config
  (defun local/helpful--navigate (button)
